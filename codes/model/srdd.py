...
'''Image Super-Resolution with Deep Dictionary ECCV2022'''
...
class Net(nn.Module):
    def __init__(self, scale=4):
        super().__init__()
        self.ps = scale  # patch size
        n_atoms = 64  # 2^N
        nfs     = [n_atoms, n_atoms*2, n_atoms*4, n_atoms*8]

        self.chsr = ESPCN(scale=scale, in_c=3, out_c=2)
    ...
    def forward(self, x, stored_dict=None, stored_code=None, shuffle=False,return_atom=False):
        b, _, h, w = x.size()
        #### initial
        ...
        m = self.ps // 2
        aa = self.ps % 2
        x[:, :, m + 2:-m - 2 - aa, m + 2:-m - 2 - aa] = self.fuse(torch.cat([x[:, :, m:-m - aa, m:-m - aa], y], dim=1))

        #### final
        x = x + xl
        x = kornia.color.ycbcr_to_rgb(torch.cat([x, xc], dim=1))
        if return_atom:
            return x, d[:1, :, :self.ps, :self.ps], c[:1, :, :1, :1]
        else:
            return x
class Timer(object):
    """A simple timer."""
