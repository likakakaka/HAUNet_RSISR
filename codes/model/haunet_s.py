...
def make_model(args, parent=False):
    return HAUNet(up_scale=args.scale[0], width=64, enc_blk_nums=[4,3],dec_blk_nums=[1,1],enc_middle_blk_num=1,dec_middle_blk_num=1)
...
class HAUNet(nn.Module):
    ...
    def __init__(self, up_scale=4, img_channel=3, width=96,enc_middle_blk_num=4,dec_middle_blk_num=1, enc_blk_nums=[4,4], dec_blk_nums=[4,4], heads = [1,2,4],):
        super().__init__()
        ...
        self.enc_middle_blks = \
            nn.Sequential(
                *[CEMBlock(chan, num_heads=heads[ii]) for _ in range(enc_middle_blk_num)]
            )
        self.dec_middle_blks = \
            nn.Sequential(
                *[CEMBlock(chan, num_heads=heads[ii]) for _ in range(dec_middle_blk_num)]
            )
        ii=0
...
if __name__ == '__main__':
    ...
    net = HAUNet(up_scale=4, width=64, enc_blk_nums=[4,3],dec_blk_nums=[1,1],enc_middle_blk_num=1,dec_middle_blk_num=1).cuda()
